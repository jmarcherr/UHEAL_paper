...
com_sub_fix = p2_fix_sub-n1_fix_sub;
...
box off
fig=gcf
saveas(fig,'/work3/jonmarc/UHEAL_paper/_eeg/_AEP/_outputs/figs/n100_age_group_fix','svg')
%%
% P2
figure('Renderer','painter')
% Young
...
fig=gcf
box off
saveas(fig,'/work3/jonmarc/UHEAL_paper/_eeg/_AEP/_outputs/figs/p200_age_group_fix','svg')
%%
% N2
figure('Renderer','painter')
% Young
...
figure('Renderer','painter')
% Young
plot(rates,nanmean(n2_sub_lat(YNH_idx,:)),'color',y_col)
hold on
plot(rates,nanmean(n2_sub_lat(MNH_idx,:)),'color',m_col)
plot(rates,nanmean(n2_sub_lat(ONH_idx,:)),'-','color',o_col)
for ii=1:4
    eb(2,ii)=errorbar(rates(ii)-.01,nanmean(n2_sub_lat(YNH_idx,ii)),nanstd(n2_sub_lat(YNH_idx,ii))/sqrt(length(YNH_idx)),'^','color',y_col,'markerfacecolor',y_col,'MarkerEdgecolor','k','MarkerSize',6.8)
    eb(2,ii)=errorbar(rates(ii),nanmean(n2_sub_lat(MNH_idx,ii)),nanstd(n2_sub_lat(MNH_idx,ii))/sqrt(length(MNH_idx)),'sq','color',m_col,'markerfacecolor',m_col,'MarkerEdgecolor','k','MarkerSize',6.8)
    eb(1,ii)=errorbar(rates(ii)+.01,nanmean(n2_sub_lat(ONH_idx,ii)),nanstd(n2_sub_lat(ONH_idx,ii))/sqrt(length(ONH_idx)),'o','color',o_col,'markerfacecolor',o_col,'MarkerEdgecolor','k')
...
set(gca,'fontsize',12)
set(gcf,'position',[438 10 270 266])
set(gca,'xtick',rates,'ytick',[0.3 0.35 0.4])
xlim([0.5 2.5])
ylim([0.3 0.4])
fig=gcf
box off
saveas(fig,'/work3/jonmarc/UHEAL_paper/_eeg/_AEP/_outputs/figs/n200_age_group_latency','svg')
%%
% P2-N1 complex
% P2
...
hold on
for ii=1:4
    eb(1,ii)=errorbar(rates(ii)-.01,nanmean(com_sub_fix(YNH_idx,ii)),nanstd(com_sub_fix(YNH_idx,ii))/sqrt(length(YNH_idx)),'^','color',y_col,'markerfacecolor',y_col,'MarkerEdgecolor','k','MarkerSize',6.8)
    eb(2,ii)=errorbar(rates(ii),nanmean(com_sub_fix(MNH_idx,ii)),nanstd(com_sub_fix(MNH_idx,ii))/sqrt(length(MNH_idx)),'sq','color',m_col,'markerfacecolor',m_col,'MarkerEdgecolor','k','MarkerSize',6.8)
    eb(3,ii)=errorbar(rates(ii)+.01,nanmean(com_sub_fix(ONH_idx,ii)),nanstd(com_sub_fix(ONH_idx,ii))/sqrt(length(ONH_idx)),'o','color',o_col,'markerfacecolor',o_col,'MarkerEdgecolor','k')
...
for ii=1:length(com_sub_fix)
    pfit(ii,:) = polyfit(rates',com_sub_fix(ii,:)',1);
...
fig = gcf;
saveas(fig,'/work3/jonmarc/UHEAL_paper/_eeg/_AEP/_outputs/figs//P2N1_all_fix','svg')
    % for stats
...
this_idx = ~isnan(age') & ~isnan(mean(p2_fix_sub(:,:),2));
this_idx = find(this_idx & idx);
