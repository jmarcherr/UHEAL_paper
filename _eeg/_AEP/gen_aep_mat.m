...
com_sub = p2_fix_sub-n1_fix_sub; %obs
...
uheal_data.AEP_p1_lat = nan(size(uheal_data.subid,1),4);
 %n100
uheal_data.AEP_n1 = nan(size(uheal_data.subid,1),4);
uheal_data.AEP_n1_lat = nan(size(uheal_data.subid,1),4);
 %p200
uheal_data.AEP_p2 = nan(size(uheal_data.subid,1),4);
uheal_data.AEP_p2_lat = nan(size(uheal_data.subid,1),4);
 %n200
uheal_data.AEP_n2 = nan(size(uheal_data.subid,1),4);
uheal_data.AEP_n2_lat = nan(size(uheal_data.subid,1),4);
...
for s=1:length(n1_mean_sub)
    ...
    uheal_data.AEP_p1_lat(this_idx,:) = p1_sub_lat(s,:);
    %n1
    uheal_data.AEP_n1(this_idx,:) = n1_fix_sub(s,:);
    uheal_data.AEP_n1_lat(this_idx,:) = n1_sub_lat(s,:);
    %p2
    uheal_data.AEP_p2(this_idx,:) = p2_fix_sub(s,:);
    uheal_data.AEP_p2_lat(this_idx,:) = p2_sub_lat(s,:);
    %n2
    uheal_data.AEP_n2(this_idx,:) = n2_sub(s,:);
    uheal_data.AEP_n2_lat(this_idx,:) = n2_sub_lat(s,:);
    %p2-n1
