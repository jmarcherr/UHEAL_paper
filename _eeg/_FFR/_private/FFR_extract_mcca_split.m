...
run('FFR_mcca_preproc_split.m')

%% MCCA on training data, use for test data
% mcca on training data
% loop over splits
for tt=1:10
    nchans = 16;
    clc
    disp(['running mcca for split ' num2str(tt) '/10'])
    % do mcca on training data
    clear x
    for ii=1:length(idx_all)
        x(:,:,ii) = permute(squeeze(FFR_train(idx_all(ii),tt,1:16,tidx{1})),[3,2,1]);
    end
    ...
    for ss=1:size(x,3)
        ...
        % training data
        z_sub = squeeze(FFR_train(idx_all(ss),tt,1:16,:))'*AA{(ss)}(:,:);
        % test data
        z_sub_test = squeeze(FFR_test(idx_all(ss),tt,1:16,:))'*AA{(ss)}(:,:);
        % noise data
        z_sub_noise = squeeze(FFR_noise(idx_all(ss),1:16,:))'*AA{(ss)}(:,:);

        % get mixing weights for first 9 components (only for 16 channels)
            for cc=1:9
                a_all(ss,cc,:) = squeeze(FFR_train(idx_all(ss),tt,1:16,tidx{1}))*nt_normcol(z(:,cc))/size(data_z,3);
            end
        ...
        % test data
        z_sub_test(:,setdiff(1:size(z,2),c_idx)) = 0;
        % noise data
        z_sub_noise(:,setdiff(1:size(z,2),c_idx)) = 0;

        %transform back to electrode space
        % training
        data_z(idx_all(ss),:,:) = permute(z_sub*pinv(AA{ss}(:,:)),[2,1]);
        % test
        data_z_test(idx_all(ss),:,:) = permute(z_sub_test*pinv(AA{ss}(:,:)),[2,1]);
        % noise
        data_z_noise(idx_all(ss),:,:) = permute(z_sub_noise*pinv(AA{ss}(:,:)),[2,1]);

        % get mixing weights (see mcca_demo3)
        a(ss,:)=squeeze(FFR_train(idx_all(ss),tt,1:16,tidx{1}))*nt_normcol(z(:,1))/size(data_z,3);
        a_test(ss,:) = squeeze(FFR_test(idx_all(ss),tt,1:16,tidx{1}))*nt_normcol(z(:,1))/size(data_z_test,3);
        ...
        % test data
        [f_tmp,fft_sub_tmp,~,~,~,SNR_tmp,~]=get_fft(squeeze(data_z_test(idx_all(ss),:,tidx{1})),foi,fs(s));
        SNR_chan_mcca(idx_all(ss),:) = SNR_tmp;
        % get channel average SNR/FFR over remaining channels
        [~,FFR_avg_tmp,~,SNR_avg_tmp,~,sig_idx_avg_tmp,~]=get_fft_chaoi(f_tmp,fft_sub_tmp,1:16,foi);
        FFR_mcca_test(idx_all(ss)) = FFR_avg_tmp;
        SNR_mcca_test(idx_all(ss)) = SNR_avg_tmp;
        sig_mcca_test(idx_all(ss)) = sig_idx_avg_tmp;

        % noise data
        [f_tmp,fft_sub_tmp,~,~,~,SNR_tmp,~]=get_fft(squeeze(data_z_noise(idx_all(ss),:,tidx{1})),foi,fs(s));
        SNR_chan_mcca(idx_all(ss),:) = SNR_tmp;
        % get channel average SNR/FFR over remaining channels
        [~,FFR_avg_tmp,~,SNR_avg_tmp,~,sig_idx_avg_tmp,~]=get_fft_chaoi(f_tmp,fft_sub_tmp,1:16,foi);
        FFR_mcca_noise(idx_all(ss)) = FFR_avg_tmp;
        SNR_mcca_noise(idx_all(ss)) = SNR_avg_tmp;
        sig_mcca_noise(idx_all(ss)) = sig_idx_avg_tmp;        

        %get fft FFR raw
        % training
        [f_tmp,fft_sub_tmp,f_fft_noise_tmp,FFR_tmp,F_tmp,SNR_tmp,F_crit_tmp]=get_fft(squeeze(FFR_train(idx_all(ss),tt,:,tidx{1})),foi,fs(s));
        ...
        % test
        [f_tmp,fft_sub_tmp,f_fft_noise_tmp,FFR_tmp,F_tmp,SNR_tmp,F_crit_tmp]=get_fft(squeeze(FFR_test(idx_all(ss),tt,:,tidx{1})),foi,fs(s));
        SNR_chan_raw(idx_all(ss),:) = SNR_tmp;
        % get channel average SNR/FFR over remaining channels
        [f_fft_avg,FFR_avg_tmp,F_avg_tmp,SNR_avg_tmp,F_crit_avg_tmp,sig_idx_avg_tmp,noise_avg_tmp]=get_fft_chaoi(f_tmp,fft_sub_tmp,1:16,foi);
        FFR_raw_test(idx_all(ss)) = FFR_avg_tmp;
        SNR_raw_test(idx_all(ss)) = SNR_avg_tmp;
        sig_raw_test(idx_all(ss)) = sig_idx_avg_tmp;

        % noise
        [f_tmp,fft_sub_tmp,f_fft_noise_tmp,FFR_tmp,F_tmp,SNR_tmp,F_crit_tmp]=get_fft(squeeze(FFR_noise(idx_all(ss),:,tidx{1})),foi,fs(s));
        SNR_chan_raw(idx_all(ss),:) = SNR_tmp;
        % get channel average SNR/FFR over remaining channels
        [f_fft_avg,FFR_avg_tmp,F_avg_tmp,SNR_avg_tmp,F_crit_avg_tmp,sig_idx_avg_tmp,noise_avg_tmp]=get_fft_chaoi(f_tmp,fft_sub_tmp,1:16,foi);
        FFR_raw_noise(idx_all(ss)) = FFR_avg_tmp;
        SNR_raw_noise(idx_all(ss)) = SNR_avg_tmp;
        sig_raw_noise(idx_all(ss)) = sig_idx_avg_tmp;
    ...
    n_sig_noise(tt) = length(find(sig_raw_noise(idx_all)));
    % test
    n_sig_mcca_test(tt) = length(find(sig_mcca_test(idx_all)));
    n_sig_raw_test(tt) = length(find(sig_raw_test(idx_all)));
    n_sig_raw_noise(tt) = length(find(sig_raw_noise(idx_all)));
...
fsize = 11
figure(1000)
set(gcf,'Renderer','painters','position',[440 182 388 420])
cmap_mcca=cbrewer('div','BrBG',15);
cmap_trials = cbrewer('div','BrBG',10);
subplot 222
%training
pm=errorbar([1 2 3],[mean(n_sig_mcca./length(idx_all)) mean(n_sig_mcca_test./length(idx_all)) mean(n_sig_noise./length(idx_all))],...'
    [std(n_sig_mcca./length(idx_all)) std(n_sig_mcca_test./length(idx_all)) std(n_sig_noise./length(idx_all))],...
    '-','Color',cmap_mcca(12,:),'LineWidth',2)
hold on
pr=errorbar([1 2 3],[mean(n_sig_raw./length(idx_all)) mean(n_sig_raw_test./length(idx_all)) mean(n_sig_raw_noise./length(idx_all))],...'
    [std(n_sig_raw./length(idx_all)) std(n_sig_raw_test./length(idx_all)) std(n_sig_raw_noise./length(idx_all))],...
    '-','Color',cmap_mcca(2,:),'LineWidth',2)
%test
%pm=bar(2,n_sig_mcca_test/length(idx_all),'FaceColor',cmap_mcca(12,:),'LineWidth',2)
%hold on
%pr=bar(2,n_sig_raw_test/length(idx_all),'FaceColor',cmap_mcca(2,:),'LineWidth',2)

xlim([0 4])
ylim([.0 1])
set(gca,'xtick',[1,2,3],'XtickLabel',{'Train','Test','Noise'},'fontsize',fsize)
...
hleg.Box = 'off'
hleg.Position = [0.7859 0.8662 0.2243 0.0772];
box off
fig = gcf;
saveas(fig,'figs/mcca_it/train_test_sig_split_noise','svg')
%%
% SNR improvement
subplot(2,2,[1 3])
for ii=1:10
