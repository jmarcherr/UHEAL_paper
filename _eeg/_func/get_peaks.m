function [abr_peaks] = get_peaks(data,t_abr)
%find ABR peaks automatic

ap_peak = find(t_abr>0.6e-3 & t_abr<2.3e-3);
...
if ~isempty(data)
        ...
        this_wv = WV_lat + wv_peak(1);
        abr_peaks.WV_latency = t_abr(this_wv);
        [abr_peaks.WV_neg,abr_peaks.WV_neg_lat] = min(squeeze(data_in(this_wv:this_wv+20)));
        abr_peaks.WV_neg_latency = t_abr(WV_neg_lat+this_wv);
    else
