function [data_out,data_out_trials,t_abr,tnidx] = ABR_correction(data_in, data_in_trials,time,tidx,fs,subid)
...
%% Baseline correction
baseline = mean(data_out(find(t_abr>-1e-3 & t_abr<0)))'; % baseline from -1 ms to 0
baseline_trials =mean(data_out_trials(find(t_abr>-1e-3 & t_abr<0),:),1);
data_out = data_out-baseline;
