function [hh,hdata,Results] = PlotACALOS_MT(value,Results,typePlot)
if exist('Results')==0
end
if contains(value,'ACALOS_BEARHL')
    HLcorrection = zeros(6,1);
    
    if endsWith(pwd,'Results\')
        
        Rawresults = load([ value '_raw.dat']);
    else
        Rawresults = load(['Results\' value '_raw.dat']);
        
    end
    validfreq = [250 500 1000 2000 4000 6000];
    %validfreq = [1000 4000 ];

    freq = unique(Rawresults(:,1));
    if sum(sum(validfreq==freq))~=6
        validData = sum(validfreq==freq); %% This indexes will be NAN data
    else
        validData = [1 1 1 1 1 1];
    end
    data_tmp = zeros(length(freq),5);
    x= [-10:2.5:120]; % THIS IS ALREADY IN DB HL as far as i understand
