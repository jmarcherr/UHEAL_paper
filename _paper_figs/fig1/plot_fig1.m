...
% new groups
y_idx_SG = find(uheal_data.Age<=25 & uheal_data.CP_X); 
m_idx_SG = find(uheal_data.Age>26 & uheal_data.Age<50 & uheal_data.CP_X)
o_idx_SG = find(uheal_data.Age>=50 & uheal_data.CP_X);
...
savepath = '/work3/jonmarc/UHEAL_paper/_paper_figs/fig1/figs/'
save_figs = 1;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
var = 'acalos_AC_slope';
labels = 'ACALOS AC slope'
lims = [0 0.75]%[-1 8];
data = uheal_data;
freqs = {'05 kHz','1 kHz','2 kHz','4 kHz'}
for ii=1:4
    data.acalos_AC_slope = uheal_data.acalos_AC_slope(:,ii);
    labels = ['ACALOS slope'];
figure
[h,ax1,ax2]=stat_plots_uh_age_pt(data,var,labels,lims)
%hleg.Position = [0.5320 0.7793 0.2155 0.1272]
...
fig = gcf;
if save_figs
    saveas(fig,[savepath 'ACALOS' freqs{ii}],'svg')
end
end
