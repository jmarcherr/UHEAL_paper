...
Varnames = {'AP','FFR','WV','ITPC','Neg. 4Hz','P2N1'}
X = {zscor_xnan(uheal_nh.AP_amp_pm);
    zscor_xnan(uheal_nh.FFR_SNR);
    zscor_xnan(uheal_nh.WV_amp_pm);
...
for ii = 1:length(QX)
    ...
    elseif ii==3
        t=text(QX(ii)-0.2,QY(ii)+0.1,Varnames{ii})
    elseif QX(ii)<0
...
fig=gcf;
%cd(root)
saveas(fig,['figs/pca_eeg_clean_WV'],'svg')
...
fnames = {'AP_amp_pm','FFR_SNR','WV_amp_pm','ITPC_ratio','Neg_4Hz','AEP_p2n1_int'}
...
%
pnames = {'AP','FFR','WV','ITPC','Neg. 4Hz','P2-N1'}
%yidx = [1,6,11,16,21]
...
%saveas(fig,['figs/corr_eeg_WV'],'svg')
