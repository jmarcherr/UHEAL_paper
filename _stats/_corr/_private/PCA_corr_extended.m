...
Varnames = {'AP','FFR','ITPC','Mean Neg.','P2 adapt.','N1','P2'}
...
for ii = 1:length(QX)
    ...
    if ii==6
        t=text(QX(ii)-0,QY(ii)-0,Varnames{ii})
    elseif ii==7
        t=text(QX(ii)-0.4,QY(ii),Varnames{ii})
    elseif ii==5
        t=text(QX(ii)-0.3,QY(ii)+0.15,Varnames{ii})
    elseif ii==3
        t=text(QX(ii)-0.1,QY(ii)+0.1,Varnames{ii})
    elseif ii==4
        t=text(QX(ii)-0.5,QY(ii)+0.1,Varnames{ii})
    elseif QX(ii)<0
...
fig=gcf;
%cd(root)
saveas(fig,['figs/pca_eeg_clean_extended'],'svg')
...
%{'AP','FFR','ITPC','Mean Neg.','P2_{adapt.}','N1','P2'}
fnames = {'AP_amp_pm','FFR_SNR','ITPC_ratio','Neg_4Hz','tone_p2_1vsrest','AEP_n1','AEP_p2'}
...
for jj=1:length(fnames)
    ...
    xlim([-3 3])
    ylim([-3 3])
    if jj+(ii-1)*length(fnames) == length(fnames)*(length(fnames)-1)
        set(gca,'fontsize',12)
    else
...
%
pnames = Varnames%{'AP','FFR','ITPC','Neg. 4Hz','N1','P2'}
%yidx = [1,6,11,16,21]
yidx = [1,7,12,17,22,31]
for ii=1:length(fnames)
    subplot(length(fnames),length(fnames),ii)
    t = title(pnames{ii},'FontSize',10,'FontName','Arial','Rotation',0)
...
saveas(fig,['figs/corr_eeg_extended'],'svg')
...
eegnames = {'AP_amp_pm','FFR_SNR','ITPC_ratio','Neg_4Hz','tone_p2_1vsrest','AEP_n1','AEP_p2'};
peegnames = {'AP','FFR','ITPC','Neg. 4Hz','P2 adapt','N1','P2'};
...
for ii=1:length(clinnames)
    for jj=1:length(eegnames)
        ...
        subplot(length(eegnames),length(clinnames),cc)
...
for ii=1:length(clinnames)
    subplot(length(eegnames),length(clinnames),ii)
    title(peegnames{ii})
end
cc = 0;
...
for pp = 1:length(p_plot)
    ...
    subplot(length(eegnames),length(clinnames),pp)
