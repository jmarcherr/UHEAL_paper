...
# load data
setwd('~/Documents/_postdoc/UHEAL_paper/_statsR')
...
out=NULL
age_tab=NULL
for (c in c('rds','nesi','tts','ssq12_mean')){
...
rownames(age_tab) <-c('rds','nesi','tts','ssq12')
age_tab

out<-kable(age_tab)
write.csv2(age_tab, "age_tab_clinprof.csv")
...
rownames(sex_tab) <-c('rds','nesi','tts','ssq12')
sex_tab

out<-kable(sex_tab)
write.csv2(sex_tab, "sex_tab_clinprof.csv")

#age_tab = cbind(est_age,err_age,t_age,p_val_age,p_corr_age)
#print(p.adjust(p_val_sex,method='bonferroni'))
#write.csv2(age_tab, "age_tab.csv")
...
out=NULL
age_tab=NULL
for (c in c('acalos_AC_slope_1','acalos_AC_slope_2','acalos_AC_slope_3','acalos_AC_slope_4')){
  temp=print(summary(lm(paste(c,"~ Age+sex"), data=uheal_data)))
  # age values
  est_age = rbind(est_age,summary(lm(paste(c,"~ Age+sex"), data=uheal_data))$coefficients[2,1])
  err_age = rbind(err_age,summary(lm(paste(c,"~ Age+sex"), data=uheal_data))$coefficients[2,2])
...
age_tab <-data.frame(df = df_all,
                     ...
                     p_corr = p_corr_age,
                     p_corr_hoch = p_corr_age_hoch)
rownames(age_tab) <-c('acalos_slope_1','acalos_slope_2','acalos_slope_3','acalos_slope_4')
age_tab

out<-kable(age_tab)
write.csv2(age_tab, "age_tab_ACALOS.csv")
...
sex_tab <-data.frame(df = df_all,
                     ...
                     p_corr = p_corr_sex,
                     p_corr_hoch = p_corr_sex_hoch)
rownames(sex_tab) <-c('acalos_slope_1','acalos_slope_2','acalos_slope_3','acalos_slope_4')
sex_tab

out<-kable(sex_tab)
write.csv2(sex_tab, "sex_tab_ACALOS.csv")
